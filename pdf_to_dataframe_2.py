...
# Obtidos inspecionando a posição das palavras do cabeçalho e valores reais no PDF.
#
# Observação: Os valores REAIS aparecem em posições DIFERENTES do texto do header!
# - Crédito (maioria): x0 ≈ 536-553 (variação observada: 536.2 a 553.2)
# - Saldo:             x0 ≈ 630-650 (alinhado com header Saldo Atual)
# ---------------------------------------------------------------------------
...
COL_CREDITO_X   = (530, 565)   # "Credito" (valores reais consolidados) x0≈536-553
COL_SALDO_X     = (630, 660)   # "Saldo Atual" (valores reais) x0≈630-650
        ...
        for num_pagina, page in enumerate(pdf.pages, 1):
            ...
            for top, ws in linhas:
                ...
                if _e_linha_conta(ws):
                    ...
                    # Filtra palavras até a coluna de Crédito (evita "Saldo Anterior")
                    ws_desc = [w for w in ws if w['x0'] < COL_CREDITO_X[0]]
                    linha_str = ' '.join(
                ...
                if _e_linha_dados(ws):
                    ...
                    credito_txt = _col(ws, *COL_CREDITO_X)  # x0≈530-565 (consolidada)
                    saldo_txt   = _col(ws, *COL_SALDO_X)    # x0≈630-660

                    # Extrai valor de crédito (um único valor por linha)
                    credito_consolidado = (
                        _br_para_float(credito_txt) if VALOR_RE.match(credito_txt) else None
                    )
